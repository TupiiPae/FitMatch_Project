...
export default function Sport_Create() {
  ...
  const scrollToFirstError = (errs) => {
    const order = [["name", refName], ["image", refImageBox], ["caloriePerRep", refCal], ["imageUrl", refImgUrl], ["descriptionHtml", refDesc]];
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      nav("/exercises/sport", { state: { justCreated: true, createdId } });
    } catch (err) {
  ...
  return (
    <div className="strength-create-page">
      <nav className="breadcrumb-nav" aria-label="breadcrumb">
        ...
        <span className="separator">/</span>
        <span className="current-page">Tạo môn thể thao (Sport)</span>
      </nav>

      <div className="card">
        <div className="page-head">
          <h2>Tạo môn thể thao (Sport)</h2>
          <div className="head-actions">
            <button type="button" className="btn ghost" onClick={() => nav(-1)}>Hủy</button>
            <button type="submit" form="sport-create-form" className="btn primary" disabled={saving}>
              {saving ? "Đang lưu..." : "Lưu"}
            </button>
          </div>
        </div>

        <form id="sport-create-form" className="sc-form-layout" onSubmit={onSubmit}>
          {/* LEFT */}
          <div className="sc-layout-left">
            ...
            <div className="sc-image-box" role="button" tabIndex={0} onClick={pickImage} ref={refImageBox}>
              {imgPreview ? (
                ...
                <img src={f.imageUrl} alt="Xem trước" onError={() => setImgPreview(null)} />
              ) : (
