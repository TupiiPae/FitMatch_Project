// admin-app/src/pages/pagesExercises/Exercises_Edit/Exercises_Edit.jsx
...
const TYPE_UI = {
  Strength: {
    icon: "fa-dumbbell",
    heading: "Chỉnh sửa bài tập",
  },
  Cardio: {
    icon: "fa-person-running",
    heading: "Chỉnh sửa bài tập (Cardio)",
  },
  Sport: {
    icon: "fa-football",
    heading: "Chỉnh sửa môn thể thao (Sport)",
  },
};
...
export default function ExercisesEdit() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (alive && resM && Array.isArray(resM) && resM.length) {
          setMuscleOptions(resM);
        }
      } catch {}
      try {
        const resE = await listEquipments().catch(() => null);
        if (alive && resE && Array.isArray(resE) && resE.length) {
          setEquipmentOptions(resE);
        }
      } catch {}
    })();
    return () => {
  ...
  useEffect(() => {
    return () => {
      if (imgPreview && String(imgPreview).startsWith("blob:")) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      // upload video mới nếu có (Strength / Cardio)
      if (!isSport && videoFile) {
        try {
          const newUrl = await uploadExerciseVideoApi(id, videoFile);
          setF((s) => ({ ...s, videoUrl: newUrl || s.videoUrl }));
        } catch (e2) {
          console.error("Upload video fail:", e2);
          toast.warning(
            "Đã lưu thông tin, nhưng upload video thất bại. Bạn có thể thử lại sau."
          );
        }
      }

      const successMsg = isSport
        ? "Cập nhật môn thể thao thành công!"
  ...
  const type = f.type || "Strength";
  const isSport = type === "Sport";
  const ui = TYPE_UI[type] || TYPE_UI.Strength;
  ...
  const nameLabel = isSport ? "Tên môn *" : "Tên bài tập *";
    ...
    <div className="strength-create-page">
      <nav className="breadcrumb-nav" aria-label="breadcrumb">
        ...
        <span className="current-group">
          <i className={`fa-solid ${ui.icon}`} aria-hidden="true" />
          <span>Quản lý Bài tập</span>
        </span>
        <span className="separator">/</span>
        <span className="current-page">{ui.heading}</span>
      </nav>
      ...
      <div className="card">
        ...
        >
                ...
                >
                  ...
                  >
                    {videoFile
                      ...
                      : "Tải lên file video (Tối đa 50MB)"}
                    <input
                      ref={videoInputRef}
                      type="file"
                      accept="video/*"
          ...
          <div className="sc-layout-right">
            ...
            <Box sx={rowGrid2}>
              <Box>
                <div className="sc-field-title">{nameLabel}</div>
                <TextField
                  inputRef={refName}
                  label={nameLabel}
                  value={f.name}
              ...
              <Box>
                ...
                <FormControl fullWidth error={!!errors.primaryMuscles}>
                  ...
                  <Select
                    ...
                    onChange={(e) =>
                      onChange("primaryMuscles", e.target.value)
                    }
                  ...
                  >
                    {muscleOptions.map((m) => (
                      <MenuItem key={m} value={m}>
                        <Checkbox
                          checked={f.primaryMuscles.indexOf(m) > -1}
                        />
              ...
              <Box>
                ...
                <FormControl fullWidth>
                  ...
                  <Select
                    ...
                    onChange={(e) =>
                      onChange("secondaryMuscles", e.target.value)
                    }
                  ...
                  >
                    {muscleOptions.map((m) => (
                      <MenuItem key={m} value={m}>
                        <Checkbox
                          checked={f.secondaryMuscles.indexOf(m) > -1}
                        />
                        <ListItemText primary={m} />
                      </MenuItem>
                    ))}
            ...
            <Box sx={rowGrid3}>
              <Box>
                ...
                <FormControl fullWidth error={!!errors.equipment}>
                  ...
                  <Select
                    ...
                    value={f.equipment}
                    onChange={(e) =>
                      onChange("equipment", e.target.value)
                    }
                    MenuProps={menuProps}
                  >
              ...
              <Box>
                ...
                <TextField
                  ...
                  onChange={(e) =>
                    onChange("caloriePerRep", e.target.value)
                  }
