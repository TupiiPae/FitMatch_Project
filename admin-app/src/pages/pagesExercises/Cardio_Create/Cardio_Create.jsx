...
import {
  createExercise, createExerciseFile, listMuscleGroups, listEquipments,
  uploadExerciseVideoApi
} from "../../../lib/api";
import "./Cardio_Create.css";
...
export default function Cardio_Create() {
  ...
  useEffect(() => () => { if (imgPreview?.startsWith?.("blob:")) URL.revokeObjectURL(imgPreview); }, [imgPreview]);
  ...
  const showImgFromUrl = () => { if (!imgFile) setImgPreview(f.imageUrl || null); };

  const scrollToFirstError = (errs) => {
    const order = [["name", refName],["image", refImageBox],["caloriePerRep", refCal],["imageUrl", refImgUrl],["videoUrl", refVidUrl],["guideHtml", refGuide],["descriptionHtml", refDesc]];
    const first = order.find(([k]) => errs[k]);
    if (first?.[1]?.current) {
      try { first[1].current.scrollIntoView({ behavior: "smooth", block: "center" }); first[1].current.focus?.(); } catch {}
    }
  };
  ...
  const validate = () => {
    ...
    else if (!/^[\p{L}\p{M}\s0-9'’\-.,()\/]+$/u.test(name)) errs.name = "Tên chỉ gồm chữ, số, khoảng trắng và ' - . , ( ) /";
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    const errs = validate();
    if (Object.keys(errs).length) { toast.error("Vui lòng kiểm tra lại các dữ liệu nhập."); scrollToFirstError(errs); return; }
    ...
    try {
      ...
      if (videoFile && createdId) {
        try { await uploadExerciseVideoApi(createdId, videoFile); }
        catch (e) { console.error(e); toast.warning("Tạo thành công, nhưng upload video thất bại. Bạn có thể thêm video sau."); }
      }

      toast.success("Tạo bài tập thành công!");
      nav("/exercises/cardio", { state: { justCreated: true, createdId } });
    } catch (err) {
  ...
  const rowGrid2 = { display: "grid", gap: 4, gridTemplateColumns: { xs: "1fr", md: "1fr 1fr" }, alignItems: "start" };
  const rowGrid3 = { display: "grid", gap: 4, gridTemplateColumns: { xs: "1fr", md: "1fr 1fr 1fr" }, alignItems: "start" };
  ...
  return (
    <div className="strength-create-page">
      <nav className="breadcrumb-nav" aria-label="breadcrumb">
        ...
        <span className="separator">/</span>
        <span className="current-page">Tạo bài tập (Cardio)</span>
      </nav>

      <div className="card">
        <div className="page-head">
          <h2>Tạo bài tập (Cardio)</h2>
          <div className="head-actions">
            <button type="button" className="btn ghost" onClick={() => nav(-1)}>Hủy</button>
            <button type="submit" form="cardio-create-form" className="btn primary" disabled={saving}>
              {saving ? "Đang lưu..." : "Lưu"}
            </button>
          </div>
        </div>

        <form id="cardio-create-form" className="sc-form-layout" onSubmit={onSubmit}>
          {/* LEFT */}
          <div className="sc-layout-left">
            ...
            <div className="sc-image-box" role="button" tabIndex={0} onClick={pickImage} ref={refImageBox}>
              {imgPreview ? <img src={imgPreview} alt="Xem trước" />
                : f.imageUrl ? <img src={f.imageUrl} alt="Xem trước" onError={() => setImgPreview(null)} />
          ...
          <div className="sc-layout-right">
            ...
            <Box sx={rowGrid2}>
              <Box>
                ...
                <TextField inputRef={refName} label="Tên bài tập *" value={f.name} onChange={(e)=>onChange("name", e.target.value)} error={!!errors.name} helperText={errors.name} fullWidth />
              </Box>
              ...
              <Box>
                ...
                <FormControl fullWidth error={!!errors.type}>
                  ...
                  <Select labelId="type-label" label="Phân loại *" value={f.type} onChange={(e)=>onChange("type", e.target.value)} MenuProps={{ disableScrollLock: true }}>
              ...
              <Box>
                ...
                <FormControl fullWidth error={!!errors.primaryMuscles}>
                  ...
                  <Select
                    ...
                    renderValue={(selected)=>(<Box sx={{ display:"flex", gap:0.5, flexWrap:"wrap" }}>{selected.map(val => <Chip key={val} label={val} size="small" />)}</Box>)}
            ...
            <Box sx={rowGrid3}>
              <Box>
                ...
                <FormControl fullWidth error={!!errors.equipment}>
                  ...
                  <Select labelId="equip-label" label="Dụng cụ *" value={f.equipment} onChange={(e)=>onChange("equipment", e.target.value)} MenuProps={{ disableScrollLock: true }}>
              ...
              <Box>
                ...
                <FormControl fullWidth error={!!errors.level}>
                  ...
                  <Select labelId="level-label" label="Mức độ *" value={f.level} onChange={(e)=>onChange("level", e.target.value)} MenuProps={{ disableScrollLock: true }}>
