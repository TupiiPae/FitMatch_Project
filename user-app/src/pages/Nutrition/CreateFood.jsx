// src/pages/Nutrition/CreateFood.jsx
import { useEffect, useState } from "react";
import { useNavigate } from "react-router-dom";
import {
  createFood as createFoodJSON,
  createFoodWithImage,
  searchFoods,
  updateFood,
  deleteFood,
} from "../../api/foods";
import "./CreateFood.css";
import { toast } from "react-toastify";

const UNIT_OPTIONS = ["g", "ml"];

export default function CreateFood() {
  const nav = useNavigate();

  // --------- FORM STATE ---------
  const [form, setForm] = useState({
    name: "",
    massG: "",
    unit: "g",
    portionName: "",
    kcal: "",
    proteinG: "",
    carbG: "",
    fatG: "",
    saltG: "",
    sugarG: "",
    fiberG: "",
  });
  const up = (k, v) => setForm((s) => ({ ...s, [k]: v }));

  // Thumbnail (1 ảnh)
  const [file, setFile] = useState(null);
  const [preview, setPreview] = useState("");
  const onPickFile = (e) => {
    const f = e.target.files?.[0];
    if (!f) return;
    setFile(f);
    setPreview(URL.createObjectURL(f));
  };
  const clearThumb = () => {
    setFile(null);
    if (preview) URL.revokeObjectURL(preview);
    setPreview("");
  };
  useEffect(() => {
    return () => {
      if (preview) URL.revokeObjectURL(preview);
    };
  }, [preview]);

  const [msg, setMsg] = useState("");

  // --------- SUBMIT ---------
  async function submit(e) {
    if (e?.preventDefault) e.preventDefault();
    setMsg("");

    // Chuẩn hoá
    const name = String(form.name || "").trim();
    const unit = form.unit === "ml" ? "ml" : "g";
    const massRaw = String(form.massG ?? "").trim();
    const massNum = massRaw === "" ? NaN : Number(massRaw);

    // Validate cứng
    if (!name) {
      setMsg("Tên món là bắt buộc");
      return;
    }
    if (!Number.isFinite(massNum) || massNum <= 0) {
      setMsg("Khối lượng (g/ml) phải là số dương");
      return;
    }

    // Helper: ép number hoặc trả null
    const numOrNull = (v, parseFn = Number) => {
      const s = String(v ?? "").trim();
      if (s === "") return null;
      const n = parseFn(s);
      return Number.isFinite(n) ? n : null;
    };

    const payload = {
      name,
      massG: massNum,
      unit,
      portionName: String(form.portionName || "").trim() || undefined,
      kcal: numOrNull(form.kcal),
      proteinG: numOrNull(form.proteinG),
      carbG: numOrNull(form.carbG),
      fatG: numOrNull(form.fatG),
      saltG: numOrNull(form.saltG),
      sugarG: numOrNull(form.sugarG),
      fiberG: numOrNull(form.fiberG),
      sourceType: "user_submitted",
    };

    try {
      if (file) {
        const fd = new FormData();
        fd.append("image", file);
        Object.entries(payload).forEach(([k, v]) => {
          if (v !== null && v !== undefined) fd.append(k, String(v));
        });
        await createFoodWithImage(fd);
      } else {
        await createFoodJSON(payload);
      }
      toast.success("Đã gửi yêu cầu. Vui lòng đợi admin duyệt.");
      setTimeout(() => nav("/dinh-duong/ghi-lai"), 800);
    } catch (err) {
      const m = err?.response?.data?.message || "Gửi duyệt thất bại";
      setMsg(m);
      toast.error(m);
    }
  }

  // --------- “Món do tôi tạo” ---------
  const [mine, setMine] = useState([]);
  const [loadingMine, setLoadingMine] = useState(false);

  async function loadMine() {
    setLoadingMine(true);
    try {
      const { data } = await searchFoods({
        onlyMine: true,
        scope: "all",
        limit: 20,
        skip: 0,
      });
      setMine(data?.items || []);
    } finally {
      setLoadingMine(false);
    }
  }
  useEffect(() => {
    loadMine();
  }, []);

  const statusLabel = (it) => {
    const s = it.status || it.approvalStatus || it.moderationStatus || "pending";
    if (s === "approved") return "Thành công";
    if (s === "rejected") return "Từ chối";
    if (s === "pending") return "Đang duyệt";
    return s;
  };
  const statusClass = (it) => {
    const s = it.status || it.approvalStatus || it.moderationStatus || "pending";
    return s === "approved" ? "ok" : s === "rejected" ? "bad" : "pending";
  };

  // --------- Inline Edit ---------
  const [editId, setEditId] = useState(null);
  const [edit, setEdit] = useState(null);

  const openEdit = (it) => {
    setEditId(it._id);
    setEdit({
      name: it.name || "",
      portionName: it.portionName || "",
      massG: it.massG ?? "",
      unit: it.unit || "g",
      kcal: it.kcal ?? "",
      proteinG: it.proteinG ?? "",
      carbG: it.carbG ?? "",
      fatG: it.fatG ?? "",
      saltG: it.saltG ?? "",
      sugarG: it.sugarG ?? "",
      fiberG: it.fiberG ?? "",
    });
  };
  const cancelEdit = () => {
    setEditId(null);
    setEdit(null);
  };
  const saveEdit = async () => {
    try {
      const payload = {
        ...edit,
        massG: Number(edit.massG),
        kcal: edit.kcal === "" ? null : Number(edit.kcal),
        proteinG: edit.proteinG === "" ? null : Number(edit.proteinG),
        carbG: edit.carbG === "" ? null : Number(edit.carbG),
        fatG: edit.fatG === "" ? null : Number(edit.fatG),
        saltG: edit.saltG === "" ? null : Number(edit.saltG),
        sugarG: edit.sugarG === "" ? null : Number(edit.sugarG),
        fiberG: edit.fiberG === "" ? null : Number(edit.fiberG),
      };
      await updateFood(editId, payload);
      toast.success("Đã lưu chỉnh sửa");
      cancelEdit();
      loadMine();
    } catch (err) {
      toast.error(err?.response?.data?.message || "Lưu thất bại");
    }
  };

  const onDelete = async (id) => {
    if (!window.confirm("Xóa món này?")) return;
    try {
      await deleteFood(id);
      toast.success("Đã xóa món");
      loadMine();
    } catch (err) {
      toast.error(err?.response?.data?.message || "Xóa thất bại");
    }
  };

  // --------- VIEW ---------
  return (
    <div className="cf-wrap">
      {/* Header */}
      <div className="cf-header">
        <h2>Tạo món ăn mới</h2>
        <div className="cf-actions">
          <button className="btn ghost" type="button" onClick={() => nav(-1)}>
            Hủy
          </button>
          <button className="btn primary" type="button" onClick={submit}>
            Tạo món ăn
          </button>
        </div>
      </div>

      {/* GRID: Left (thumb) – Right (form) */}
      <form className="cf-grid" onSubmit={submit}>
        {/* LEFT */}
        <aside className="cf-left">
          <div className="card thumb-card">
            <div className="card-title">Thumbnail</div>
            <div className="thumb-box">
              {preview ? (
                <img src={preview} alt="preview" className="thumb-img" />
              ) : (
                <div className="thumb-placeholder">
                  <i className="fa-regular fa-image"></i>
                </div>
              )}
            </div>
            <div className="thumb-ctl">
              <label className="btn light">
                <input type="file" hidden accept="image/*" onChange={onPickFile} />
                Chọn ảnh
              </label>
              {preview && (
                <button type="button" className="btn ghost" onClick={clearThumb}>
                  Xóa ảnh
                </button>
              )}
            </div>
            <div className="hint">Chỉ 1 ảnh (*.png, *.jpg, *.jpeg)</div>
          </div>

          <div className="card status-card">
            <div className="card-title">Trạng thái duyệt</div>
            <div className="status-dot green"></div>
            <div className="hint">
              Món mới sẽ ở trạng thái <b>Đang duyệt</b>.
            </div>
          </div>
        </aside>

        {/* RIGHT */}
        <section className="cf-right">
          <div className="card">
            <div className="tab-title">Thông tin chung</div>

            <div className="row">
              <label>
                Tên món <span className="req">*</span>
              </label>
              <input
                className="ipt"
                value={form.name}
                onChange={(e) => up("name", e.target.value)}
                placeholder="Ví dụ: Ức gà áp chảo"
                required
              />
            </div>

            <div className="row two">
              <div>
                <label>Khẩu phần (tùy chọn)</label>
                <input
                  className="ipt"
                  value={form.portionName}
                  onChange={(e) => up("portionName", e.target.value)}
                  placeholder="VD: 1 phần / 1 lon / 1 lát..."
                />
              </div>
            </div>

            <div className="row two">
              <div>
                <label>
                  Khối lượng <span className="req">*</span>
                </label>
                <input
                  className="ipt"
                  type="number"
                  min="0"
                  step="1"
                  value={form.massG}
                  onChange={(e) => up("massG", e.target.value)}
                  placeholder="VD: 100"
                  required
                />
              </div>
              <div>
                <label>Đơn vị</label>
                <div className="unit-toggle">
                  {UNIT_OPTIONS.map((u) => (
                    <button
                      key={u}
                      type="button"
                      className={`unit ${form.unit === u ? "on" : ""}`}
                      onClick={() => up("unit", u)}
                    >
                      {u.toUpperCase()}
                    </button>
                  ))}
                </div>
              </div>
            </div>
          </div>

          <div className="card">
            <div className="tab-title">Giá trị dinh dưỡng</div>

            <div className="row three">
              <div>
                <label>Calo (cal)</label>
                <input
                  className="ipt"
                  type="number"
                  step="1"
                  min="0"
                  value={form.kcal}
                  onChange={(e) => up("kcal", e.target.value)}
                />
              </div>
              <div>
                <label>Đạm (g)</label>
                <input
                  className="ipt"
                  type="number"
                  step="0.1"
                  min="0"
                  value={form.proteinG}
                  onChange={(e) => up("proteinG", e.target.value)}
                />
              </div>
              <div>
                <label>Đường bột (g)</label>
                <input
                  className="ipt"
                  type="number"
                  step="0.1"
                  min="0"
                  value={form.carbG}
                  onChange={(e) => up("carbG", e.target.value)}
                />
              </div>
            </div>

            <div className="row three">
              <div>
                <label>Chất béo (g)</label>
                <input
                  className="ipt"
                  type="number"
                  step="0.1"
                  min="0"
                  value={form.fatG}
                  onChange={(e) => up("fatG", e.target.value)}
                />
              </div>
              <div>
                <label>Muối (g)</label>
                <input
                  className="ipt"
                  type="number"
                  step="0.1"
                  min="0"
                  value={form.saltG}
                  onChange={(e) => up("saltG", e.target.value)}
                />
              </div>
              <div>
                <label>Đường (g)</label>
                <input
                  className="ipt"
                  type="number"
                  step="0.1"
                  min="0"
                  value={form.sugarG}
                  onChange={(e) => up("sugarG", e.target.value)}
                />
              </div>
            </div>

            <div className="row two">
              <div>
                <label>Chất xơ (g)</label>
                <input
                  className="ipt"
                  type="number"
                  step="0.1"
                  min="0"
                  value={form.fiberG}
                  onChange={(e) => up("fiberG", e.target.value)}
                />
              </div>
            </div>

            {msg && <div className="form-msg">{msg}</div>}
            <div className="form-actions">
              <button type="button" className="btn ghost" onClick={() => nav(-1)}>
                Hủy
              </button>
              <button type="submit" className="btn primary">
                Tạo món ăn
              </button>
            </div>
          </div>
        </section>
      </form>

      {/* ====== LIST: Món tạo bởi tôi ====== */}
      <div className="mine-wrap">
        <div className="card">
          <div className="tab-title">Món do tôi tạo</div>

          {loadingMine ? (
            <div className="muted">Đang tải...</div>
          ) : (
            <div className="mine-list">
              {mine.map((it) => (
                <div key={it._id} className="mine-item">
                  <img
                    className="mi-thumb"
                    src={it.imageUrl || "/images/food-placeholder.jpg"}
                    alt={it.name}
                  />
                  <div className="mi-info">
                    <div className="mi-top">
                      <div className="mi-name">{it.name}</div>
                      <span className={`badge ${statusClass(it)}`}>{statusLabel(it)}</span>
                    </div>

                    <div className="mi-sub">
                      {(it.portionName || "Khẩu phần tiêu chuẩn")} · {(it.massG ?? "-")}{" "}
                      {it.unit || "g"} · {(it.kcal ?? "-")} cal
                    </div>

                    {editId === it._id ? (
                      <div className="edit-grid">
                        <input
                          className="ipt"
                          value={edit.name}
                          onChange={(e) => setEdit((d) => ({ ...d, name: e.target.value }))}
                        />
                        <input
                          className="ipt"
                          type="number"
                          value={edit.massG}
                          onChange={(e) => setEdit((d) => ({ ...d, massG: e.target.value }))}
                        />
                        <select
                          className="ipt"
                          value={edit.unit}
                          onChange={(e) => setEdit((d) => ({ ...d, unit: e.target.value }))}
                        >
                          {UNIT_OPTIONS.map((u) => (
                            <option key={u} value={u}>
                              {u.toUpperCase()}
                            </option>
                          ))}
                        </select>
                        <input
                          className="ipt"
                          type="number"
                          value={edit.kcal}
                          onChange={(e) => setEdit((d) => ({ ...d, kcal: e.target.value }))}
                        />
                        <button className="btn small" type="button" onClick={saveEdit}>
                          Lưu
                        </button>
                        <button className="btn ghost small" type="button" onClick={cancelEdit}>
                          Hủy
                        </button>
                      </div>
                    ) : (
                      <div className="mi-actions">
                        <button className="btn light" type="button" onClick={() => openEdit(it)}>
                          Chỉnh sửa
                        </button>
                        <button className="btn ghost" type="button" onClick={() => onDelete(it._id)}>
                          Xóa
                        </button>
                      </div>
                    )}
                  </div>
                </div>
              ))}
              {mine.length === 0 && <div className="muted">Chưa có món nào do bạn tạo.</div>}
            </div>
          )}
        </div>
      </div>
    </div>
  );
}
