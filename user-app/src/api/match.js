...
// formData: cover(file), name, description, ageRange, gender, trainingFrequency, maxMembers, joinPolicy
export async function createTeamRoom(formData){
  const res = await api.post("/match/groups", formData); 
  return unwrap(res);
}
...
export async function getRoomDetail(id){
  const res = await api.get(`/match/rooms/${id}`);
  return unwrap(res);
}

export async function leaveMatchRoom(id){
